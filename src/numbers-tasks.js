...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const angleInRadians = dotProduct / (length1 * length2);
  const limitedAngleInRadians = Math.max(-1, Math.min(1, angleInRadians));
  return Math.acos(limitedAngleInRadians);
}
...
function getSumOfDigits(num) {
  let sum = 0;
  const numSrt = num.toString();

  for (let i = 0; i < numSrt.length; i += 1) {
    sum += parseInt(numSrt[i], 10);
  }
...
function numberToStringInBase(number, base) {
  const a = typeof number === 'number' ? number : parseInt(number, base);
...
function toFixed(number, fractionDigits) {
  ...
  return roundedNumber.toFixed(fractionDigits);
}
